...
#include "../src/P182/P182_internal.h"
...
{ // Tests for the field arithmetic
    ...
    printf("\n--------------------------------------------------------------------------------------------------------\n\n"); 
    printf("Testing field arithmetic over GF(p182): \n\n"); 

    // Field addition over the prime p182
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fprandom182_test(a); fprandom182_test(b); fprandom182_test(c); fprandom182_test(d); fprandom182_test(e); fprandom182_test(f); 

        // printf("a=\t0x%016lx%016lx%016lx\n", a[2], a[1], a[0]);
        // printf("b=\t0x%016lx%016lx%016lx\n", b[2], b[1], b[0]);
        // printf("c=\t0x%016lx%016lx%016lx\n", c[2], c[1], c[0]);
        // printf("d=\t0x%016lx%016lx%016lx\n", d[2], d[1], d[0]);
        // printf("e=\t0x%016lx%016lx%016lx\n", e[2], e[1], e[0]);
        // printf("f=\t0x%016lx%016lx%016lx\n", f[2], f[1], f[0]);



        fpadd182(a, b, d); fpadd182(d, c, e);                 // e = (a+b)+c
        fpadd182(b, c, d); fpadd182(d, a, f);                 // f = a+(b+c)
        fpcorrection182(e);
        fpcorrection182(f);
        if (compare_words(e, f, NWORDS_FIELD)!=0) { passed=0; break; }

        fpadd182(a, b, d);                                     // d = a+b 
        fpadd182(b, a, e);                                     // e = b+a
        fpcorrection182(d);
        fpcorrection182(e);
        if (compare_words(d, e, NWORDS_FIELD)!=0) { passed=0; break; }

        fpzero182(b);
        fpadd182(a, b, d);                                     // d = a+0 
        if (compare_words(a, d, NWORDS_FIELD)!=0) { passed=0; break; }
        
        fpzero182(b);
        fpcopy182(a, d);     
        fpneg182(d);                      
        fpadd182(a, d, e);                                     // e = a+(-a)
        fpcorrection182(e);
    ...
    // Field subtraction over the prime p182
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fprandom182_test(a); fprandom182_test(b); fprandom182_test(c); fprandom182_test(d); fprandom182_test(e); fprandom182_test(f); 

        fpsub182(a, b, d); fpsub182(d, c, e);                 // e = (a-b)-c
        fpadd182(b, c, d); fpsub182(a, d, f);                 // f = a-(b+c)
        fpcorrection182(e);
        fpcorrection182(f);
        if (compare_words(e, f, NWORDS_FIELD)!=0) { passed=0; break; }

        fpsub182(a, b, d);                                     // d = a-b 
        fpsub182(b, a, e);                                         
        fpneg182(e);                                           // e = -(b-a)
        fpcorrection182(d);
        fpcorrection182(e);
        ...
        fpzero182(b);
        fpsub182(a, b, d);                                     // d = a-0 
        if (compare_words(a, d, NWORDS_FIELD)!=0) { passed=0; break; }
        
        fpzero182(b);
        fpcopy182(a, d);                 
        fpsub182(a, d, e);                                     // e = a+(-a)
        fpcorrection182(e);
        if (compare_words(e, b, NWORDS_FIELD)!=0) { passed=0; break; }
    }
    ...
    // Field multiplication over the prime p182
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {    
        fprandom182_test(a); fprandom182_test(b); fprandom182_test(c);  
        fprandom182_test(ma); fprandom182_test(mb); fprandom182_test(mc); fprandom182_test(md); fprandom182_test(me); fprandom182_test(mf); 
        ...
        fpmul182_mont(ma, mb, md); fpmul182_mont(md, mc, me);                          // e = (a*b)*c
        fpmul182_mont(mb, mc, md); fpmul182_mont(md, ma, mf);                          // f = a*(b*c)
        from_mont(me, e);
        from_mont(mf, f);
        if (compare_words(e, f, NWORDS_FIELD)!=0) { passed=0; break; }
      
        to_mont(a, ma); to_mont(b, mb); to_mont(c, mc); 
        fpadd182(mb, mc, md); fpmul182_mont(ma, md, me);                               // e = a*(b+c)
        fpmul182_mont(ma, mb, md); fpmul182_mont(ma, mc, mf); fpadd182(md, mf, mf);    // f = a*b+a*c
        from_mont(me, e);
        from_mont(mf, f);
        if (compare_words(e, f, NWORDS_FIELD)!=0) { passed=0; break; }
       
        to_mont(a, ma); to_mont(b, mb);
        fpmul182_mont(ma, mb, md);                                                      // d = a*b 
        fpmul182_mont(mb, ma, me);                                                      // e = b*a 
        from_mont(md, d);
        from_mont(me, e);
        if (compare_words(d, e, NWORDS_FIELD)!=0) { passed=0; break; }
        
        to_mont(a, ma);
        fpzero182(b); b[0] = 1; to_mont(b, mb);
        fpmul182_mont(ma, mb, md);                                                      // d = a*1  
        from_mont(ma, a);
        from_mont(md, d);                
        if (compare_words(a, d, NWORDS_FIELD)!=0) { passed=0; break; }
        
        fpzero182(b); to_mont(b, mb);
        fpmul182_mont(ma, mb, md);                                                      // d = a*0  
        from_mont(mb, b);
        from_mont(md, d);                
    ...
    // Field squaring over the prime p182
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fprandom182_test(a);
        ...
        to_mont(a, ma);
        fpsqr182_mont(ma, mb);                                 // b = a^2
        fpmul182_mont(ma, ma, mc);                             // c = a*a 
        if (compare_words(mb, mc, NWORDS_FIELD)!=0) { passed=0; break; }

        fpzero182(a); to_mont(a, ma);
        fpsqr182_mont(ma, md);                                 // d = 0^2 
    ...
    // Field inversion over the prime p182
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fprandom182_test(a); 
        to_mont(a, ma);
        fpzero182(d); d[0]=1; to_mont(d, md);
        fpcopy182(ma, mb);                            
        fpinv182_mont(ma);                                
        fpmul182_mont(ma, mb, mc);                             // c = a*a^-1 
...
{ // Tests for the quadratic extension field arithmetic
    ...
    printf("\n--------------------------------------------------------------------------------------------------------\n\n"); 
    printf("Testing quadratic extension arithmetic over GF(p182^2): \n\n"); 

    // Addition over GF(p182^2)
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fp2random182_test((digit_t*)a); fp2random182_test((digit_t*)b); fp2random182_test((digit_t*)c); fp2random182_test((digit_t*)d); fp2random182_test((digit_t*)e); fp2random182_test((digit_t*)f); 

        fp2add182(a, b, d); fp2add182(d, c, e);                 // e = (a+b)+c
        fp2add182(b, c, d); fp2add182(d, a, f);                 // f = a+(b+c)
        if (compare_words((digit_t*)e, (digit_t*)f, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2add182(a, b, d);                                     // d = a+b 
        fp2add182(b, a, e);                                     // e = b+a
        if (compare_words((digit_t*)d, (digit_t*)e, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2zero182(b);
        fp2add182(a, b, d);                                     // d = a+0 
        if (compare_words((digit_t*)a, (digit_t*)d, 2*NWORDS_FIELD)!=0) { passed=0; break; }
        
        fp2zero182(b);
        fp2copy182(a, d);     
        fp2neg182(d);                      
        fp2add182(a, d, e);                                     // e = a+(-a)
    ...
    // Subtraction over GF(p182^2)
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fp2random182_test((digit_t*)a); fp2random182_test((digit_t*)b); fp2random182_test((digit_t*)c); fp2random182_test((digit_t*)d); fp2random182_test((digit_t*)e); fp2random182_test((digit_t*)f); 

        fp2sub182(a, b, d); fp2sub182(d, c, e);                 // e = (a-b)-c
        fp2add182(b, c, d); fp2sub182(a, d, f);                 // f = a-(b+c)
        if (compare_words((digit_t*)e, (digit_t*)f, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2sub182(a, b, d);                                     // d = a-b 
        fp2sub182(b, a, e);                                         
        fp2neg182(e);                                           // e = -(b-a)
        if (compare_words((digit_t*)d, (digit_t*)e, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2zero182(b);
        fp2sub182(a, b, d);                                     // d = a-0 
        if (compare_words((digit_t*)a, (digit_t*)d, 2*NWORDS_FIELD)!=0) { passed=0; break; }
        
        fp2zero182(b);
        fp2copy182(a, d);                 
        fp2sub182(a, d, e);                                     // e = a+(-a)
        if (compare_words((digit_t*)e, (digit_t*)b, 2*NWORDS_FIELD)!=0) { passed=0; break; }
    }
    ...
    // Multiplication over GF(p182^2)
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {    
        fp2random182_test((digit_t*)a); fp2random182_test((digit_t*)b); fp2random182_test((digit_t*)c);  
        fp2random182_test((digit_t*)ma); fp2random182_test((digit_t*)mb); fp2random182_test((digit_t*)mc); fp2random182_test((digit_t*)md); fp2random182_test((digit_t*)me); fp2random182_test((digit_t*)mf); 
        ...
        fp2mul182_mont(ma, mb, md); fp2mul182_mont(md, mc, me);                          // e = (a*b)*c
        fp2mul182_mont(mb, mc, md); fp2mul182_mont(md, ma, mf);                          // f = a*(b*c)
        from_fp2mont(me, e);
        from_fp2mont(mf, f);
        if (compare_words((digit_t*)e, (digit_t*)f, 2*NWORDS_FIELD)!=0) { passed=0; break; }
      
        to_fp2mont(a, ma); to_fp2mont(b, mb); to_fp2mont(c, mc); 
        fp2add182(mb, mc, md); fp2mul182_mont(ma, md, me);                               // e = a*(b+c)
        fp2mul182_mont(ma, mb, md); fp2mul182_mont(ma, mc, mf); fp2add182(md, mf, mf);   // f = a*b+a*c
        from_fp2mont(me, e);
        from_fp2mont(mf, f);
        if (compare_words((digit_t*)e, (digit_t*)f, 2*NWORDS_FIELD)!=0) { passed=0; break; }
       
        to_fp2mont(a, ma); to_fp2mont(b, mb);
        fp2mul182_mont(ma, mb, md);                                                      // d = a*b 
        fp2mul182_mont(mb, ma, me);                                                      // e = b*a 
        from_fp2mont(md, d);
        from_fp2mont(me, e);
        if (compare_words((digit_t*)d, (digit_t*)e, 2*NWORDS_FIELD)!=0) { passed=0; break; }
        
        to_fp2mont(a, ma);
        fp2zero182(b); b[0][0] = 1; to_fp2mont(b, mb);
        fp2mul182_mont(ma, mb, md);                                                      // d = a*1  
        from_fp2mont(md, d);               
        if (compare_words((digit_t*)a, (digit_t*)d, 2*NWORDS_FIELD)!=0) { passed=0; break; }
        
        fp2zero182(b); to_fp2mont(b, mb);
        fp2mul182_mont(ma, mb, md);                                                      // d = a*0 
        from_fp2mont(md, d);               
    ...
    // Squaring over GF(p182^2)
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fp2random182_test((digit_t*)a);
        
        to_fp2mont(a, ma);
        fp2sqr182_mont(ma, mb);                                 // b = a^2
        fp2mul182_mont(ma, ma, mc);                             // c = a*a 
        from_fp2mont(mb, b);               
        from_fp2mont(mc, c);               
        if (compare_words((digit_t*)b, (digit_t*)c, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2zero182(a); to_fp2mont(a, ma);
        fp2sqr182_mont(ma, md);                                 // d = 0^2 
        from_fp2mont(md, d);               
        if (compare_words((digit_t*)a, (digit_t*)d, 2*NWORDS_FIELD)!=0) { passed=0; break; }
    }
    ...
    for (n=0; n<TEST_LOOPS; n++)
    {
        ...
        to_fp2mont(a, ma);
        fp2zero182(d); d[0][0]=1; to_fp2mont(d, md);
        fp2copy182(ma, mb);                            
        fp2inv182_mont(ma);                                
        fp2mul182_mont(ma, mb, mc);                             // c = a*a^-1              
        from_fp2mont(mc, c);  
...
bool fp_run()
{
    ...
    printf("\n--------------------------------------------------------------------------------------------------------\n\n"); 
    printf("Benchmarking field arithmetic over GF(p182): \n\n"); 
        
    fprandom182_test(a); fprandom182_test(b); fprandom182_test(c);

    // GF(p) addition using p182
    cycles = 0;
    ...
    // GF(p) multiplication using p182
    cycles = 0;
    for (n=0; n<BENCH_LOOPS; n++)
    {
        cycles1 = cpucycles(); 
...
bool fp2_run()
{
    ...
    printf("\n--------------------------------------------------------------------------------------------------------\n\n"); 
    printf("Benchmarking quadratic extension arithmetic over GF(p182^2): \n\n"); 
...
bool ecisog_run()
{
    ...
    for (n=0; n<BENCH_LOOPS; n++)
    {
        fp2random182_test((digit_t*)A24); fp2random182_test((digit_t*)C24);
    ...
    for (n=0; n<BENCH_LOOPS; n++)
    {
        fp2random182_test((digit_t*)A); fp2random182_test((digit_t*)coeff[0]); fp2random182_test((digit_t*)coeff[1]); fp2random182_test((digit_t*)coeff[2]);
    ...
    for (n=0; n<BENCH_LOOPS; n++)
    {
        fp2random182_test((digit_t*)A4); fp2random182_test((digit_t*)C);
    ...
    for (n=0; n<BENCH_LOOPS; n++)
    {
        fp2random182_test((digit_t*)A); fp2random182_test((digit_t*)C);
...
int main()
{
    ...
    OK = OK && fp_test();          // Test field operations using p182
    OK = OK && fp_run();           // Benchmark field operations using p182

    OK = OK && fp2_test();         // Test arithmetic functions over GF(p182^2)
    OK = OK && fp2_run();          // Benchmark arithmetic functions over GF(p182^2)
